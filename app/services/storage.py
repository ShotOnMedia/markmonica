...
def _ensure_cors(client) -> bool:
    ...
    try:
        client.put_bucket_cors(Bucket=settings.s3_bucket, CORSConfiguration={"CORSRules": [{"AllowedHeaders": ["*"], "AllowedMethods": ["GET", "HEAD", "PUT"], "AllowedOrigins": [origin], "ExposeHeaders": ["ETag"], "MaxAgeSeconds": 3600}]})
        logger.info("Configured CORS for object storage bucket '%s'", settings.s3_bucket)
        return True
    except ClientError as exc:
        code = str(exc.response.get("Error", {}).get("Code", ""))
        logger.warning("Unable to configure CORS for bucket '%s' via S3 API (%s); continuing because bucket CORS can be configured separately", settings.s3_bucket, code or exc.__class__.__name__)
        return False
...
def create_presigned_download(object_key: str, expires_in: int = 900, filename: str | None = None, response_filename: str | None = None) -> str:
...
def delete_objects(object_keys: list[str | None]) -> None:
    ...
    for start in range(0, len(keys), 1000):
        ...
        if errors:
            raise RuntimeError("Object deletion failed: " + "; ".join(f"{e.get('Key')}: {e.get('Code')}" for e in errors[:5]))
